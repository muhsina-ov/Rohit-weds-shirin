...
// The invitation verse reveals itself paragraph by paragraph and word by word
    ...
    <div className="flex flex-col gap-5 text-center">
      {paragraphs.map((p, pIdx) => {
        ...
        return (
          <motion.p
            key={pIdx}
            className="font-display text-xl sm:text-2xl italic leading-relaxed text-[#f5eee2]/90"
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-80px" }}
            variants={{
              hidden: {},
              show: { transition: { staggerChildren: 0.04, delayChildren: pIdx * 0.15 } },
            }}
          >
            {words.map((w, i) => (
              <motion.span
